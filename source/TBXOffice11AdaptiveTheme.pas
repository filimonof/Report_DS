...
//
// Modified OfficeXP theme by Roy Magne Klever - I named it 'Office11XPtheme'
//
// 07.02.2003 - Roy Magne Klever roy.magne@os.ino.no
//              Added gradient to toolbars
//              Changed statusbar and draghandels...
// 01.04.2003 - Roy Magne Klever roy.magne@os.ino.no
//              A lot of changes... Gradient in popupmenu
//              Please note this theme is best viewed with xp themes
//              active... not classic win9X win2000 Changed!
//              To use this theme in win95 - win2000 set UseAdaptive to false
...
var
  ...
  CounterLock: Integer;
  gradCol1, gradCol2, gradHandle1, gradHandle2, gradHandle3, gradBL, gradTL: TColor;
  GradientBmp: TBitmap;
...
begin
  ...
  begin
    if (AViewType and TVT_MENUBAR) = TVT_MENUBAR then Result := TBXAdaptiveInfo.MenubarColor
...
function TTBXOffice11AdaptiveTheme.GetBtnColor(const ItemInfo: TTBXItemInfo; ItemPart: TItemPart): TColor;
const
...
function TTBXOffice11AdaptiveTheme.GetPartColor(const ItemInfo: TTBXItemInfo; ItemPart: TItemPart): TColor;
const
...
begin
  ...
  if Direction = tGTopBottom then begin
    ...
    begin
      ...
      Windows.FillRect(Canvas.Handle, Rect(ARect.Left, ARect.Bottom - Counter - 1, ARect.Right, ARect.Bottom - Counter), Brush);
      DeleteObject(Brush);
    end;
  end else begin
    ...
    begin
      Brush := CreateSolidBrush(
        ...
        Byte(gc1 + (((gc2 - gc1) * (Counter)) div GSize)),
        Byte(bc1 + (((bc2 - bc1) * (Counter)) div GSize))));
      Windows.FillRect(Canvas.Handle, Rect(ARect.Right - Counter - 1, ARect.Top, ARect.Right - Counter, ARect.Bottom), Brush);
      DeleteObject(Brush);
    end;
  end;
end;

function TTBXOffice11AdaptiveTheme.GetItemColor(const ItemInfo: TTBXItemInfo): TColor;
begin
  Result := GetPartColor(ItemInfo, ipBody);
  if Result = clNone then Result := GetViewColor(ItemInfo.ViewType);
end;

function TTBXOffice11AdaptiveTheme.GetItemTextColor(const ItemInfo: TTBXItemInfo): TColor;
begin
  Result := GetPartColor(ItemInfo, ipText);
end;

function TTBXOffice11AdaptiveTheme.GetItemImageBackground(const ItemInfo: TTBXItemInfo): TColor;
begin
  Result := GetBtnColor(ItemInfo, ipBody);
  if Result = clNone then result := GetViewColor(ItemInfo.ViewType);
end;

procedure TTBXOffice11AdaptiveTheme.GetViewBorder(ViewType: Integer; out Border: TPoint);
const
...
procedure TTBXOffice11AdaptiveTheme.GetMargins(MarginID: Integer; out Margins: TTBXMargins);
begin
  with Margins do
    case MarginID of
      MID_TOOLBARITEM:
...
procedure TTBXOffice11AdaptiveTheme.PaintBackgnd(Canvas: TCanvas; const ADockRect, ARect, AClipRect: TRect;
...
begin
  ...
  begin
    ...
    //or ((AViewType and TVT_MENUBAR) = TVT_MENUBAR)
    then
    begin
    ...
    begin
      ...
      begin
        IntersectRect(R, ARect, AClipRect);
        FillGradient(Canvas, R, UseGradCol2, UseGradCol1, TGLeftRight);
      end else
      }
      //begin
...
procedure TTBXOffice11AdaptiveTheme.PaintCheckMark(Canvas: TCanvas; ARect: TRect; const ItemInfo: TTBXItemInfo);
var
  X, Y: Integer;
begin
...
procedure TTBXOffice11AdaptiveTheme.PaintChevron(Canvas: TCanvas; ARect: TRect; const ItemInfo: TTBXItemInfo);
const
...
procedure TTBXOffice11AdaptiveTheme.PaintEditButton(Canvas: TCanvas; const ARect: TRect;
...
procedure TTBXOffice11AdaptiveTheme.PaintButton(Canvas: TCanvas; const ARect: TRect; const ItemInfo: TTBXItemInfo);
var
  R: TRect;
begin
  with ItemInfo, Canvas do
  begin
    ...
    begin
      ...
      if ComboPart = cpSplitRight then Dec(R.Left);
      // rmk
      if (Selected) and ((ViewType and VT_TOOLBAR) = VT_TOOLBAR) then begin
        if IsVertical then
          FillGradient(Canvas, R, GetBtnColor(ItemInfo, ipBody), gradCol2, TGLeftRight)
        else
          FillGradient(Canvas, R, GetBtnColor(ItemInfo, ipBody), gradCol2, TGTopBottom);
      end else
...
procedure TTBXOffice11AdaptiveTheme.PaintFloatingBorder(Canvas: TCanvas; const ARect: TRect;
...
procedure TTBXOffice11AdaptiveTheme.PaintFrame(Canvas: TCanvas; const ARect: TRect;
...
procedure TTBXOffice11AdaptiveTheme.PaintMDIButton(Canvas: TCanvas; ARect: TRect;
...
procedure TTBXOffice11AdaptiveTheme.PaintMenuItem(Canvas: TCanvas; var ARect: TRect;
...
procedure TTBXOffice11AdaptiveTheme.PaintPopupNCArea(Canvas: TCanvas; R: TRect; const PopupInfo: TTBXPopupInfo);
var
  PR: TRect;
begin
  with Canvas do
...
procedure TTBXOffice11AdaptiveTheme.PaintSeparator(Canvas: TCanvas; ARect: TRect;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Right := Right - 2;
        Bottom := Top + 1;
        DrawLineEx(Canvas, Left, Top, Right, Top, clWhite);
        Top := Top - 1;
        Left := Left - 1;
        Right := Right - 1;
        DrawLineEx(Canvas, Left, Top, Right, Top, gradCol1);
      end else begin
        Top := Bottom div 2;
    ...
    begin
      ...
      Bottom := Bottom - 2;
      Left := Right div 2;
      DrawLineEx(Canvas, Left, Top, Left, Bottom, clWhite);
      Top := Top - 1;
      Left := Left - 1;
      Bottom := Bottom - 1;
      DrawLineEx(Canvas, Left, Top, Left, Bottom, gradCol1);
    end;
  end;
end;
...
procedure TTBXOffice11AdaptiveTheme.PaintToolbarNCArea(Canvas: TCanvas; R: TRect;
  ...
  begin
    ...
    begin
      with R do begin
        ...
        // rmk 1.4.2003
        Pen.Color := gradTL; //MenubarColor;//gradCol1;
        MoveTo(Left, Top);
        LineTo(Left, Top + 1);
        LineTo(Left + 2, Top - 1);
    ...
    begin
      Canvas.Brush.Color := GradTL; //MenubarColor;
      Canvas.FillRect(R);
      //FillGradient(Canvas, R, MenuBarColor, MenuBarColor, TGLeftRight);
      InflateRect(R, -2, -2);
    end;
    //  InflateRect(R, -2, -2);
    ...
    { Drag Handle }
    c := gradHandle1; //clBtnFace;  clBtnShadow
    Brush.Color := c;
...
{    if (ToolbarInfo.ViewType and TVT_MENUBAR) <> TVT_MenuBar then
    begin
      if (R.Right < R.Bottom) then begin
        ...
        with R do
        begin
          Pixels[Left, Top]:= gradCol1;
          Pixels[Left, Bottom - 2]:= gradCol1;
        end;
        R.Bottom:= R.Bottom - 1;
        R.Left:= R.Left + 1
      end else begin
...
procedure TTBXOffice11AdaptiveTheme.PaintDockPanelNCArea(Canvas: TCanvas; R: TRect; const DockPanelInfo: TTBXDockPanelInfo);
...
begin
  ...
  begin
    ...
    begin
      FrameRectEx(Canvas, R, TBXAdaptiveInfo.MenubarColor {clBtnFace}, True);
    ...
    // rmk 1.4.2003
    HeaderColor := TBXAdaptiveInfo.MenubarColor; //clBtnFace;
...
begin
  ...
  if USE_THEMES then
  begin
    gradCol1 := NearestMixedColor(clHighLight, c1, 136);
    gradCol2 := NearestMixedColor(clHighLight, clWhite, 8);
    gradHandle1 := NearestMixedColor(clHighLight, clWhite, 48);
    gradHandle2 := Blend(clHighLight, clBlack, 50);
    gradHandle3 := c1;
    gradBL := clHighLight;
...
procedure TTBXOffice11AdaptiveTheme.PaintGlyph(Canvas: TCanvas; R: TRect; Kind, State: Integer);
var
  X, Y: Integer;
...
procedure TTBXOffice11AdaptiveTheme.PaintStatusBar(Canvas: TCanvas; R: TRect; Part: Integer);
var
...
begin
  with Canvas do
    case Part of
      SBP_BODY:
        begin
          FillRectEx(Canvas, R, TBXAdaptiveInfo.MenubarColor);
